(function () {
  ...
  socket.on("connect", () => {
    ...
    try {
      socket.emit("join", {});
      if (
        typeof CONVERSATION_IDS !== "undefined" &&
    ...
    try {
      ...
      if (fromOther) {
        ...
        if (conversationsGrid && conversationsGrid.firstChild !== convEl) {
          ...
          setTimeout(() => {
            // Move to top
            conversationsGrid.insertBefore(
              convEl,
              conversationsGrid.firstChild
            );
      ...
      console.error("chat_index new_message handler error", e);
    }
  });
  ...
  socket.on("read", (d) => {
    try {
      ...
      console.error("chat_index read handler error", e);
    }
  });
  ...
  function attachClickHandlers() {
    document
      .querySelectorAll("[data-conv-id] .conversation-link")
      .forEach((a) => {
        ...
        a.addEventListener("click", (ev) => {
          try {
            ...
            if (convId) {
              fetch("/api/conversations/" + convId + "/mark_read", {
                method: "POST",
  ...
  document
    .querySelectorAll(".conversation-card, .helper-card")
    .forEach((card) => {
  ...
  function performSearch() {
    ...
    conversationCards.forEach((card) => {
      ...
      if (name) {
        ...
        const previewText = preview
          ? preview.textContent.toLowerCase().trim()
          : "";
    ...
    const conversationsSection = document.querySelector(
      ".conversations-section"
    );
    const conversationsGrid = document.querySelector(".conversations-grid");
    let noResultsMsg = conversationsSection
      ? conversationsSection.querySelector(".no-results-message")
      : null;
    ...
    let helperNoResultsMsg = helpersSection
      ? helpersSection.querySelector(".no-results-message")
      : null;
